...
/**
 * Bulk practice data for exercising the alumni + job posting features
 * together: 50 alumni accounts (each with a resume — skills, experience,
 * certifications — so they're real candidates, not empty shells), an
 * AlumniId record per account spread across every claim-status stage, and
 * a batch of job applications against the existing approved job postings
 * so the job board / applicants list / alumni ID management pages all have
 * real, connected data to click through rather than one seeder building
 * disconnected rows.
 *
 * Every account shares one known password so the printed email list is
 * immediately usable for logging in as any of them.
 */
class AlumniPracticeSeeder extends Seeder
{
    public const PASSWORD = 'Alumni@2026';

    private const FIRST_NAMES_MALE = [
        'Juan', 'Jose', 'Pedro', 'Miguel', 'Antonio', 'Ramon', 'Ricardo', 'Fernando', 'Manuel', 'Rafael',
        'Gabriel', 'Daniel', 'Andres', 'Emilio', 'Vicente', 'Alfredo', 'Roberto', 'Eduardo', 'Francisco', 'Marco',
        'Nathaniel', 'Christian', 'Adrian', 'Benjamin', 'Leonardo',
    ];

    private const FIRST_NAMES_FEMALE = [
        'Maria', 'Ana', 'Carla', 'Rosa', 'Liza', 'Grace', 'Ella', 'Joy', 'Angela', 'Cristina',
        'Michelle', 'Patricia', 'Katrina', 'Bianca', 'Nicole', 'Samantha', 'Jasmine', 'Kristine', 'Erica', 'Camille',
        'Diana', 'Faith', 'Hazel', 'Trisha', 'Angel',
    ];

    private const LAST_NAMES = [
        'Santos', 'Reyes', 'Cruz', 'Bautista', 'Ocampo', 'Garcia', 'Torres', 'Ramos', 'Mendoza', 'Castillo',
        'Villanueva', 'Del Rosario', 'Aquino', 'Fernandez', 'Gonzales', 'Rivera', 'Domingo', 'Pascual', 'Salazar', 'Navarro',
        'Marquez', 'Aguilar', 'Flores', 'Rosales', 'Padilla', 'Valdez', 'Manalo', 'Tolentino', 'Ignacio', 'Dizon',
    ];

    private const JOB_TITLES = [
        'Intern', 'Assistant', 'Associate', 'Staff', 'Coordinator', 'Analyst', 'Specialist', 'Officer', 'Representative', 'Clerk',
    ];

    private const CERT_NAMES = [
        'Effective Communication', 'Project Management Fundamentals', 'Customer Service Excellence', 'Data Analysis Basics',
        'Leadership and Teamwork', 'Digital Literacy', 'Occupational Safety and Health', 'First Aid and Basic Life Support',
        'Financial Literacy', 'Public Speaking',
    ];

    /** pending/under_review/ready_to_claim/claimed, weighted roughly 40/25/20/15. */
    ...
    public function run(): void
    {
        ...
        for ($i = 1; $i <= 50; $i++) {
            ...
            $email = 'alumni.practice' . $i . '@plvalumnet.test';
            $batch = fake()->numberBetween(2016, 2024);
            ...
            $alumnus = $user->alumnus()->create([
                ...
                'linkedin_url' => fake()->boolean(60) ? 'https://linkedin.com/in/' . Str::slug($firstName . '-' . $lastName . '-' . $i) : null,
            ]);
        ...
        // ── Connect them to the job posting feature: real applications with
        // real compatibility scores against the existing approved postings.
        $applied = [];
        foreach ($jobPostings as $job) {
            $applicantCount = min($alumniPool->count(), fake()->numberBetween(4, 10));
            $applicants = $alumniPool->random($applicantCount);
            ...
            foreach ($applicants as $alumnus) {
                ...
                $status = 'pending';
                if ($job->remainingHiringSlots() > 0 && fake()->boolean(15)) {
                    $status = 'hired';
        ...
        $lines = ["Password for every seeded account: " . self::PASSWORD, ''];
        foreach ($credentials as $cred) {
            $lines[] = "{$cred['email']}  |  " . self::PASSWORD . "  |  {$cred['name']}";
        }
        $outputPath = storage_path('app/alumni_practice_credentials.txt');
        file_put_contents($outputPath, implode(PHP_EOL, $lines));

        $this->command->info('Seeded 50 alumni accounts + AlumniId records + job applications.');
        $this->command->info('Shared password: ' . self::PASSWORD);
