...
<body class="bg-slate-100">
        ...
        <main class="flex-1 flex flex-col overflow-hidden min-w-0">
            ...
            <div class="flex-1 overflow-y-auto p-6">
                    ...
                    <table class="jobs-table">
                        <thead class="bg-[#0E0F3B] text-white">
                            <tr>
                                ...
                                <th class="border-r border-slate-700">Company Name <i data-lucide="chevron-down" class="inline w-3 h-3 ml-0.5 sort-icon"></i></th>
                                <th class="border-r border-slate-700">Location <i data-lucide="chevron-down" class="inline w-3 h-3 ml-0.5 sort-icon"></i></th>
                                <th class="border-r border-slate-700">Posted By <i data-lucide="chevron-down" class="inline w-3 h-3 ml-0.5 sort-icon"></i></th>
                                <th class="border-r border-slate-700">Job Type <i data-lucide="chevron-down" class="inline w-3 h-3 ml-0.5 sort-icon"></i></th>
                                <th class="border-r border-slate-700">Job Setup <i data-lucide="chevron-down" class="inline w-3 h-3 ml-0.5 sort-icon"></i></th>
                                <th class="border-r border-slate-700">Recommended Program <i data-lucide="chevron-down" class="inline w-3 h-3 ml-0.5 sort-icon"></i></th>
                                <th class="border-r border-slate-700">Status <i data-lucide="chevron-down" class="inline w-3 h-3 ml-0.5 sort-icon"></i></th>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            ...
                            <tr class="hover:bg-slate-50/80 transition-colors">
                                ...
                                <td class="font-medium text-black border-r border-slate-100">{{ $j->job_posting_address }}</td>
                                <td class="font-medium text-black border-r border-slate-100">
                                    {{ $j->employer->user->user_first_name }} {{ $j->employer->user->user_last_name }}
                                </td>
                                <td class="font-medium text-black border-r border-slate-100">{{ $j->job_posting_employment_type }}</td>
                                <td class="font-medium text-black border-r border-slate-100">{{ $j->job_posting_setup }}</td>
                                <td class="font-medium text-black border-r border-slate-100">
                                ...
                                <td class="border-r border-slate-100">
                                    <span class="px-2 py-1 rounded-full border text-[7px] font-bold bg-amber-100 text-amber-600 border-amber-200 inline-block">
                                        ● PENDING
                                    </span>
                                </td>
                                <td class="font-medium text-black border-r border-slate-100">{{ $j->job_closing_date }}</td>
                                <td class="text-center relative">
                                    <div class="inline-block text-left relative">
                                        <button class="menu-button p-1.5 hover:bg-slate-100 rounded-full transition-colors">
                                        ...
                                        <div class="action-dropdown bg-white border border-slate-200 rounded-md shadow-xl">
                                            <div class="py-1">
                                                <form action="{{ route('jobPosting.approve', $j->job_posting_id) }}" method="POST" class="w-full">
                                                    @csrf
                                                    <button type="submit" class="flex items-center w-full px-4 py-2 text-sm text-[#0E0F3B] hover:bg-green-50">
                                                        <i data-lucide="check-circle" class="w-4 h-4 mr-3 text-green-500"></i> Approve
                                                    </button>
                                                </form>
                                                <button onclick="openConfirmAction(<?= $j['id'] ?>, 'decline', '<?= htmlspecialchars($j['title']) ?>')"
                            ...
                            @empty
                            <tr>
                                <td colspan="10" class="text-center py-8 text-slate-400 text-sm">No pending job posts.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
                ...
                <div class="flex items-center mb-4 gap-3">
                    ...
                    <button onclick="exportCSV()" class="flex-shrink-0 flex items-center gap-2 px-5 py-2 bg-[#C73D1A] hover:bg-[#a83215] text-white text-xs font-bold rounded-lg transition-all uppercase">
                    ...
                    <table class="jobs-table">
                        ...
                        <tbody class="divide-y divide-slate-100" id="jobs-tbody">
                            ...
                            <tr class="hover:bg-slate-50/80 transition-colors"
                                ...
                                data-datetime="{{ $j->created_at }}"
                                data-closing="{{ $j->job_closing_date }}">
                                <td class="font-medium text-black border-r border-slate-100">{{ $j->job_posting_title }}</td>
                                <td class="font-medium text-black border-r border-slate-100">{{ $j->job_posting_company }}</td>
                                <td class="font-medium text-black border-r border-slate-100">{{ $j->job_posting_address }}</td>
                                <td class="font-medium text-black border-r border-slate-100">
                                ...
                                <td class="border-r border-slate-100">
                                    <span class="px-2 py-1 rounded-full border text-[6px] font-semibold bg-green-100 text-green-600 border-green-200 inline-block">
                                        ● APPROVED
                                    </span>
                                </td>
                                <td class="font-medium text-black border-r border-slate-100">{{ $j->job_closing_date }}</td>
                                    ...
                                    <div class="inline-block text-left relative">
                                        ...
                                        <div class="action-dropdown bg-white border border-slate-200 rounded-md shadow-xl">
                                            <div class="py-1">
                                                ...
                                                <button onclick="openConfirmAction(<?= $j['id'] ?>, 'delete', '<?= htmlspecialchars($j['title']) ?>')"
                            ...
                            @empty
                            <tr>
                                <td colspan="10" class="text-center py-8 text-slate-400 text-sm">No approved job posts.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
                    <div id="empty-state" class="hidden text-center py-12 text-slate-400 text-sm">No job posts found.</div>
                </div>
    ...
    <!-- View Job Modal -->
    <div id="viewJobModal" class="fixed inset-0 z-50 flex items-center justify-center hidden bg-black bg-opacity-50 backdrop-blur-sm">
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden transform transition-all">
            <div class="relative h-32 bg-slate-800 flex items-end p-6 overflow-hidden">
                ...
                <button onclick="closeViewModal()" class="absolute top-4 right-4 text-white/80 hover:text-white transition-colors">
    ...
    <!-- Sidebar Overlay -->
    <div id="sidebar-overlay" onclick="toggleSidebar('filter-sidebar')" class="fixed inset-0 bg-black/20 backdrop-blur-sm z-40 hidden transition-opacity"></div>

    <!-- Filter Sidebar -->
    <div id="filter-sidebar" class="fixed top-0 right-0 h-full w-[320px] bg-white shadow-2xl z-50 transform translate-x-full transition-transform duration-300 ease-in-out flex flex-col">
        ...
        <div class="flex-1 overflow-y-auto p-6 space-y-6">
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" placeholder="Enter Job Title" class="w-full border border-slate-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-[#C73D1A]">
                </div>
                <div>
                    <label class="text-[13px] font-bold text-[#1e1b4b] block mb-2">Company Name</label>
                    <input type="text" placeholder="Enter Company Name" class="w-full border border-slate-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-[#C73D1A]">
                </div>
            </div>
            <div>
                <label class="text-[13px] font-bold text-[#1e1b4b] block mb-2">Recommended Program</label>
                <select class="w-full border border-slate-300 rounded-md px-3 py-2 text-sm text-slate-600 focus:outline-none">
            ...
            <div>
                ...
                <div class="space-y-2">
                    <label class="flex items-center gap-3 text-sm text-[#1e1b4b] font-medium"><input type="checkbox" class="w-4 h-4 rounded border-slate-300"> Part-time</label>
                    <label class="flex items-center gap-3 text-sm text-[#1e1b4b] font-medium"><input type="checkbox" class="w-4 h-4 rounded border-slate-300"> Full-time</label>
                </div>
            </div>
            <hr class="border-slate-100">
            <div>
                <label class="text-[10px] font-bold text-[#C73D1A] uppercase tracking-wider block mb-3">Job Setup</label>
                <div class="space-y-2">
                    <label class="flex items-center gap-3 text-sm text-[#1e1b4b] font-medium"><input type="checkbox" class="w-4 h-4 rounded border-slate-300"> On-site</label>
                    <label class="flex items-center gap-3 text-sm text-[#1e1b4b] font-medium"><input type="checkbox" class="w-4 h-4 rounded border-slate-300"> Remote</label>
                    <label class="flex items-center gap-3 text-sm text-[#1e1b4b] font-medium"><input type="checkbox" class="w-4 h-4 rounded border-slate-300"> Hybrid</label>
                </div>
            </div>
            <hr class="border-slate-100">
            <div>
                <label class="text-[10px] font-bold text-[#C73D1A] uppercase tracking-wider block mb-3">Date Posted</label>
                <div class="grid grid-cols-2 gap-y-2">
                    <label class="flex items-center gap-2 text-xs font-medium text-[#1e1b4b]"><input type="checkbox"> Today</label>
                    <label class="flex items-center gap-2 text-xs font-medium text-[#1e1b4b]"><input type="checkbox"> Last 6 months</label>
                    <label class="flex items-center gap-2 text-xs font-medium text-[#1e1b4b]"><input type="checkbox"> This week</label>
                    <label class="flex items-center gap-2 text-xs font-medium text-[#1e1b4b]"><input type="checkbox"> This Year</label>
                    <label class="flex items-center gap-2 text-xs font-medium text-[#1e1b4b]"><input type="checkbox"> This month</label>
                </div>
            </div>
            <hr class="border-slate-100">
            <div>
                <label class="text-[10px] font-bold text-[#C73D1A] uppercase tracking-wider block mb-3">Closing Date</label>
                <div class="grid grid-cols-2 gap-y-2">
                    <label class="flex items-center gap-2 text-xs font-medium text-[#1e1b4b]"><input type="checkbox"> Today</label>
                    <label class="flex items-center gap-2 text-xs font-medium text-[#1e1b4b]"><input type="checkbox"> Last 6 months</label>
                    <label class="flex items-center gap-2 text-xs font-medium text-[#1e1b4b]"><input type="checkbox"> This week</label>
        ...
        <div class="p-6 border-t border-slate-100">
            <button onclick="applyFilters()" class="w-full bg-[#0a0a23] text-white py-3 rounded text-sm font-bold tracking-widest hover:bg-black transition-colors uppercase">
                Apply Filter
            </button>
        </div>
    </div>
